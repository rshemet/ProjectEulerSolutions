...
import itertools 
from PokerGame import CheckPair, CheckInOrder, GetMaxCard, SortCardsDescending, UpdateBestComb
...
def GetAnswer12():
    my_factors = []
    i = 1
    while len(my_factors) < 502:
        my_factors = GetFactors(GetTriangle(i))
        i += 1
    return GetTriangle(i-1)

#Question13

def GetAnswer13():

    fiftyDigitNumbers =     [37107287533902102798797998220837590246510135740250,\
    ...
    sum = 0
    for i in range(0, len(fiftyDigitNumbers)):
        sum += fiftyDigitNumbers[i]
...
def CollatzSequence(x):
    ...
    while seq[len(seq)-1] != 1:
        if seq[len(seq)-1] % 2 == 0:
            seq.append(int(seq[len(seq)-1]/2))
        else:
            seq.append(3*seq[len(seq)-1]+1)
    return seq

def GetAnswer14():
    # Not the best, takes 85sec to run
    max_collatz_length = 0
    max_collatz_length_int = 0
    for i in range(1,1000000):
        if len(CollatzSequence(i)) > max_collatz_length:
            max_collatz_length = len(CollatzSequence(i))
            max_collatz_length_int = i
    return max_collatz_length_int
...
def GetAnswer21():
    ...
    for i in range (2, 10001):
        i_factors = GetFactors(i)
        sum_i_factors = sum(i_factors) - i
        if sum(GetFactors(sum_i_factors)) - sum_i_factors == i:
            output.append([i,sum_i_factors])
    for i in output:
        if i[0] != i[1]:
            sum_res += i[0] + i[1]
    return sum_res/2
...
def GetAnswer54():
    ...
    for i in range(0,len(new_list)):
        a_new = new_list[i][0].split()
        b_new = new_list[i][1].split()
        new_new_list.append([a_new,b_new])
    
    p1scorenew = 0

    for i in new_new_list:
        player_1_comb = EvaluateHand(i[0])
        player_2_comb = EvaluateHand(i[1])
        if player_1_comb != player_2_comb:
            p1scorenew+=1
    return p1scorenew
